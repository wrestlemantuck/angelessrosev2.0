    ...
    public class PatchHandler
    {
        ...
        public static void PatchAll()
        {
            if (!IsPatched)
            {
                    ...
                    .Where(t => t != null && t.IsClass && t.GetCustomAttribute<HarmonyPatch>() != null))
                {
                    ...
                    {
                        PatchErrors++;
                        NotifiLib.SendNotification($"Failed to patch {type.FullName}: {ex} {ex.StackTrace}, this should never happen, restart your game, or report it"); // this shouldnt happen, if so, we will send a notification to the user, instead of debug logging
                    }
                }
        ...
        public static void ApplyPatch(Type targetClass, string methodName, MethodInfo prefix = null, MethodInfo postfix = null, Type[] parameterTypes = null)
        {
            var original =
                ...
                ?? throw new Exception($"Method '{methodName}' not found on {targetClass.FullName}"); // not good!
        ...
        public static void RemovePatch(Type targetClass, string methodName, Type[] parameterTypes = null) // unused, but kept for future use
        {
            var original =
                (parameterTypes == null ?
                targetClass.GetMethod(methodName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static) :
